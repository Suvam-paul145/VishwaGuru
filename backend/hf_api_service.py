...
async def detect_all_clip(image: Union[Image.Image, bytes], client: httpx.AsyncClient = None) -> Dict[str, List[Dict]]:
    """
    Optimized detection: Runs all checks in a single API call.
    """
    ...
    categories = {
        ...
        "garbage": {
            "labels": ["plastic bottle", "glass bottle", "metal can", "paper cardboard", "organic food waste", "electronic waste", "general trash"],
            "targets": ["plastic bottle", "glass bottle", "metal can", "paper cardboard", "organic food waste", "electronic waste", "general trash"]
        },
        "fire": {
            "labels": ["fire", "smoke", "flames", "burning", "normal scene", "safe"],
            "targets": ["fire", "smoke", "flames", "burning"]
    ...
    for cat_data in categories.values():
        all_labels.update(cat_data["labels"])
    ...
    # Single API Call
    results = await query_hf_api(img_bytes, unique_labels, client=client)
    ...
    # Helper to filter results for a category
    def filter_category(cat_key, res_list):
    ...
    output["vandalism"] = filter_category("vandalism", results)
    output["infrastructure"] = filter_category("infrastructure", results)
    output["flooding"] = filter_category("flooding", results)
    output["fire"] = filter_category("fire", results)
    output["garbage"] = filter_category("garbage", results)
