...
import { Camera, RefreshCw, AlertTriangle, CheckCircle, Dog } from 'lucide-react';
      ...
      {!image ? (
        <div className="relative h-full flex flex-col">
          ...
          <div className="absolute bottom-8 left-0 right-0 flex justify-center items-center gap-8">
            ...
            <button
              onClick={capture}
              className="p-4 bg-amber-600 rounded-full hover:bg-amber-700 transition shadow-lg border-4 border-white/20"
            >
              <Camera size={32} />
            </button>
          ...
          <div className="flex-1">
            {analyzing && (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-gray-300">Analyzing for stray animals...</p>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                {result.length > 0 ? (
                  result.map((det, index) => (
                    <div key={index} className="bg-gray-800 p-4 rounded-lg flex justify-between items-center border border-amber-500/30">
                      <div>
                        <p className="font-bold text-lg capitalize text-amber-400">{det.label}</p>
                  ...
                  <div className="bg-gray-800 p-4 rounded-lg flex items-center gap-3">
                    ...
                    <p>No stray animals detected.</p>
                  </div>
                )}

                {result.length > 0 && (
                    <div className="mt-4 p-4 bg-amber-900/30 border border-amber-500/50 rounded-lg">
                        <h4 className="font-bold text-amber-200 mb-2">Guidance:</h4>
                        <ul className="list-disc list-inside text-sm text-gray-300 space-y-1">
                            <li>Keep a safe distance.</li>
                            <li>Do not provoke the animal.</li>
                            <li>Report aggressive animals to municipal control.</li>
                        </ul>
                    </div>
                )}
              </div>
